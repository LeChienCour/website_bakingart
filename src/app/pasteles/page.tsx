    ...
    <>
      ...
      <section className="bg-gradient-to-br from-rosa/20 via-crema to-crema py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <Heading as="h1" size="display">Catálogo de <span className="text-rosa">Pasteles</span></Heading>
          ...
          <div className="mt-8 flex justify-center gap-4 flex-wrap">
            <Button asChild variant="brand-primary" size="brand-lg">
              <a href={waHref} target="_blank" rel="noopener noreferrer">Cotizar por WhatsApp</a>
            </Button>
          </div>
        </div>
      </section>
      ...
      <SectionWrapper background="white">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {PASTELES.map((p) => (
            <div key={p.nombre} className="rounded-2xl border border-gris-light overflow-hidden hover:shadow-lg transition-shadow duration-200 group">
              <div className="h-48 bg-gradient-to-br from-rosa/20 to-turquesa/10 flex items-center justify-center">
                <span className="text-6xl">{p.emoji}</span>
              </div>
              <div className="p-5 bg-white">
                <div className="flex items-start justify-between mb-2">
                  <h3 className="font-heading font-bold text-gris-text text-lg group-hover:text-rosa transition-colors">{p.nombre}</h3>
                ...
                <Button asChild variant="brand-outline" size="brand-sm" className="w-full">
                  <a href={waHref} target="_blank" rel="noopener noreferrer">Cotizar este pastel</a>
                </Button>
              </div>
            </div>
          ))}
      ...
      <section className="bg-gris-text py-16 text-center">
        <div className="max-w-xl mx-auto px-4">
          <h2 className="font-heading font-bold text-h2 text-crema mb-3">¿No encuentras lo que buscas?</h2>
          <Text variant="body" className="text-crema/70 mb-6">Hacemos cualquier diseño. Escríbenos y lo platicamos.</Text>
          <Button asChild variant="brand-primary" size="brand-lg">
            <a href={waHref} target="_blank" rel="noopener noreferrer">Hablar con Karenina</a>
          </Button>
        </div>
      </section>
    </>
